...
import { TRASH_PURGE_DURATION_DAY, SOFT_DELETE_PURGE_DURATION_DAY } from "@/utils/constants"
...
export async function purgeExpiredItems(): Promise<void> {
    const trashCutoff = DateTime.now().minus({ days: TRASH_PURGE_DURATION_DAY }).toISO()
    const tombstoneCutoff = DateTime.now().minus({ days: SOFT_DELETE_PURGE_DURATION_DAY }).toISO()
